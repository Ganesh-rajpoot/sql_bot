...
sql_questions = [
    ...
    },
    {
        ...
        "correct_answer": "SELECT department, MAX(salary) as max_salary FROM employees GROUP BY department",
        "sample_table": employees_table
    },
    {
        "question": "Retrieve orders with total amount between 500 and 800.",
        "correct_answer": "SELECT * FROM orders WHERE total_amount BETWEEN 500 AND 800",
        "sample_table": orders_table
    },
    {
        "question": "Use a subquery to select employees with salary above the average salary.",
        "correct_answer": "SELECT * FROM employees WHERE salary > (SELECT AVG(salary) FROM employees)",
        "sample_table": employees_table
    },
    {
        "question": "Find departments with employees who have salary above 70000.",
        "correct_answer": "SELECT DISTINCT department FROM employees WHERE salary > 70000",
        "sample_table": employees_table
    },
    {
        "question": "Calculate the total order amount for each customer.",
        "correct_answer": "SELECT customer_id, SUM(total_amount) as total_customer_spend FROM orders GROUP BY customer_id",
        "sample_table": orders_table
    },
    {
        "question": "Select employees with names starting with 'A'.",
        "correct_answer": "SELECT * FROM employees WHERE name LIKE 'A%'",
        "sample_table": employees_table
    },
    {
        "question": "Find the total number of unique departments.",
        "correct_answer": "SELECT COUNT(DISTINCT department) as unique_departments FROM employees",
        "sample_table": employees_table
    },
    {
        "question": "Get the department with the highest total salary.",
        "correct_answer": "SELECT department, SUM(salary) as total_salary FROM employees GROUP BY department ORDER BY total_salary DESC LIMIT 1",
        "sample_table": employees_table
    },
    {
        "question": "Retrieve manager names for each employee using a self-join.",
        "correct_answer": "SELECT e.name as employee, m.name as manager FROM employees e LEFT JOIN employees m ON e.manager_id = m.employee_id",
        "sample_table": pd.merge(employees_table, employees_table, left_on='manager_id', right_on='employee_id', suffixes=('', '_manager'))
    },
    {
        "question": "Find customers who have total order amounts greater than the average order total.",
        "correct_answer": "SELECT customer_id, SUM(total_amount) as total_spend FROM orders GROUP BY customer_id HAVING SUM(total_amount) > (SELECT AVG(total_amount) FROM orders)",
        "sample_table": orders_table
    }
]
